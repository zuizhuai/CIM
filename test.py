...
def Train():
    ...
    parse.add_argument('--trainsize', default=384, type=int)
    parse.add_argument('--batch_size', default=28, type=int)
    parse.add_argument('--num_works', default=4, type=int)
    parse.add_argument('--epoch', default=10, type=int)
    parse.add_argument('--lr', default=0.05, type=float)
    ...
    parse.add_argument('--save_epoch', default=1000, type=int)
    ...
    try:
        ...
        for epoch in range(args.start_epoch + 1, args.start_epoch + 1 + args.epoch):
            ...
            if epoch % args.save_epoch == 0:
                torch.save(model.state_dict(), args.save_path + 'CIM_epoch_%d.pth' % epoch)
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(args.save_path):
            os.makedirs(args.save_path)
        torch.save(model.state_dict(), args.save_path + '\\' + 'CIM_epoch_{}.pth'.format(epoch))
